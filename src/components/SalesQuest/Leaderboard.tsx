  ...
  return (
    <div className="grid gap-5 lg:grid-cols-5">
      <div className="space-y-2.5 lg:col-span-3">
        <h2 className="mb-1 flex items-center gap-2 text-sm font-bold uppercase tracking-wide text-zinc-400">
        ...
        {top.map((m, i) => {
          ...
          return (
            <Card
              key={m.id}
              className={`flex items-center gap-3 px-4 py-3 ${isYou ? "border-violet-500/40 bg-violet-500/[0.06]" : ""}`}
            >
              <div className="flex h-9 w-9 items-center justify-center text-lg">
                {MEDALS[i] ?? <span className="text-sm font-bold text-zinc-500">{i + 1}</span>}
              </div>
              <div className="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-zinc-700 to-zinc-800 text-xs font-bold">
                {m.avatar}
              </div>
              <div className="min-w-0 flex-1">
                ...
                <p className="text-[11px] text-zinc-500">Level {m.level} · {m.questsCompleted} {t("questsCompleted")}</p>
              </div>
              <p className="font-mono text-sm font-bold text-violet-300">{m.xp.toLocaleString()} XP</p>
            </Card>
          );
        })}
      </div>
      ...
      <div className="lg:col-span-2">
        ...
        <Card className="p-4">
          <p className="text-xs text-zinc-500">{t("yourRank")}</p>
          <p className="text-3xl font-black text-zinc-50">#{youRank || "—"}</p>

          {xpToFirst > 0 ? (
            <>
              <p className="mt-3 text-xs text-zinc-500">{t("toFirstPlace")}</p>
              <p className="mb-1.5 font-mono text-sm font-bold text-amber-300">{xpToFirst.toLocaleString()} XP</p>
              <Progress value={(you.xp / (leader?.xp || 1)) * 100} colorClassName="bg-gradient-to-r from-amber-400 to-amber-300" />
            </>
          ) : (
            <p className="mt-3 text-sm font-semibold text-emerald-300">{t("youAreFirst")}</p>
          )}

          <div className="mt-4 space-y-2 border-t border-white/5 pt-3 text-xs text-zinc-400">
