...
function ToastItem({ toast }: { toast: ToastMessage }) {
  ...
  return (
    <div
      className={`pointer-events-auto flex min-w-[240px] items-center gap-3 rounded-xl border ${tone.border} bg-zinc-900/95 px-4 py-3 shadow-2xl backdrop-blur-md animate-[toastIn_0.3s_ease-out]`}
    >
      ...
      <div>
        ...
        {toast.subtitle && <p className="text-xs text-zinc-500">{toast.subtitle}</p>}
      </div>
    </div>
  );
}
